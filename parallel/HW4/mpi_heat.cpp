...
int main(int argc, char *argv[]) {
    ...
    for (int k = 0; k < converged_iter - 1; k++) {
        ...
        for (int j = receive_placement[rank]; j < receive_placement[rank] + receive_count[rank]; j++) {
        ...
        flag = true;
        for (int i = 0; i < size; i++) if (!all_flag[i]) flag = false;
        for (int i = 0; i < size; i++) all_flag[i] = flag;
...
// gather output to node 0, remark it when testing performance
        if (rank != 0) {
        ...
        if (rank == 0) {
            for (int i = 1; i < size; ++i) {
                for (int j = receive_placement[i]; j < receive_placement[i] + receive_count[i]; ++j) {
    ...
    if (rank == 0) {
        if (draw) {
            ...
            fs << "mpi," << length << ',' << converged_iter << ',' << size << ',' << time_use << endl;
            fs.close();
        }
